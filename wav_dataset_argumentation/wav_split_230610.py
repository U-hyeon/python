...
def total_align(A, B):
    ...
    temp_sim, temp_A_point, temp_B_point = total_sim(A, B)
    temp_A_start = temp_A_start + temp_A_point
    temp_B_start = temp_B_start + temp_B_point
    while (temp_A_start < A.shape[1]-1) and (temp_B_start < B.shape[1]-1):
        temp_sim, temp_A_point, temp_B_point = total_sim(A[:, temp_A_start:], B[:, temp_B_start:])
        temp_A_start = int(temp_A_start + temp_A_point)
        temp_B_start = int(temp_B_start + temp_B_point)

        if temp_sim >= BOUNDARY:
            toggle = True
            r_sim = temp_sim
            A_point = temp_A_start
        else:
            if toggle:
                return r_sim, A_point

    print("current split point: ", A_point, "sim = ", r_sim)
    return r_sim, A_point
...
def total_sim(A, B):  # 전체 유사도 (numpy.ndarray, numpy.ndarray)
    ...
    A_half: int = int(A.shape[1] / 2)
    A_point: int = int(A.shape[1] / 2)
    B_point: int = int(B.shape[1] / 2)
    r_B_point: int = 0

    print("A_point:", A_point, end=" > ")
    while (A_half > 1):
        # 루프시작과 동시에 각 변수 초기화
        A_half = int(A_half / 2)
        B_point = 0
        ...
        '''
        for len_B in np.arange(0, B.shape[1], 4): # B파일 4프레임씩 추가로 포함해가며 각 부분집합과의 유사도도출
            for speed in np.arange(0.1, 3.0, 0.1):  # 0.1 ~ 3배속, 0.1 단위
                B_stretch = librosa.effects.time_stretch(B[:, :len_B], rate=speed)
        ...
        with ThreadPoolExecutor() as executor:
            results = []
            for len_B in np.arange(0, B.shape[1], 1):
                results.append(executor.submit(SIM_partial_B, A, B, len_B, A_point))
            for future in results:
                temp_sim_B, temp_B_point = future.result()
                if temp_sim_B >= BOUNDARY:
                    temp_sim_A = temp_sim_B
                    B_point = temp_B_point
...
def SIM_partial_B(A, B, len_B, A_point):  # [0]: similarity, [1]: B_point
    r_sim: float = -1
    '''
    for speed in np.arange(0.1, 3.0, 0.1):  # 0.1 ~ 3배속, 0.1 단위
        B_stretch = librosa.effects.time_stretch(B[:, :len_B], rate=speed)
        r_sim = cos_sim(A[:, :int(A_point)], B_stretch)
    '''
    with ThreadPoolExecutor() as executor:
        results = []
        for speed in np.arange(0.1, 3.0, 0.1):  # 0.1 ~ 3배속, 0.1 단위
            results.append(executor.submit(SIM_time_stretch, A[:, :int(A_point)], B[:, :len_B], speed))
        for future in results:
...
def SIM_time_stretch(A_part, B_part, speed):
    r_sim: float = -1

    B_stretch = librosa.effects.time_stretch(B_part, rate=speed)
    r_sim = cos_sim(A_part, B_stretch)
...
path1 = os.path.join("ㄱㅏㅅ", "1.wav")
path2 = os.path.join("ㄱㅏㅅ", "1.wav")
path3 = os.path.join("ㅜ", "1.wav")
...
mfcc_C = librosa.feature.mfcc(y=C, sr=sr3, hop_length=hop_length, n_mfcc=13)

s1 = mfcc_A[:, :20]
s2 = mfcc_B[:, :20]

t1, t3 = extend_array_set(mfcc_A, mfcc_C)
t2, t3 = extend_array_set(mfcc_B, mfcc_C)
...
plt.show()
librosa.display.specshow(t3, sr=sr3, hop_length=hop_length, x_axis='time')
plt.show()
