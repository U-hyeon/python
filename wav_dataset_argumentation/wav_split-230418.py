...
def extend_column_with_silence(arr, column):
    # 새로운 열 추가
...
def total_sim(X, Y): # 전체 유사도 (numpy.ndarray, numpy.ndarray)
    # 초기화
    r = -1

    for speed in np.arange(0.1, 3.0, 0.1): # 0.1 ~ 3배속, 0.1 단위
        X_stretch = librosa.effects.time_stretch(X, rate=speed)

        # mfcc

        temp = cos_sim(X_stretch, Y)
        if (temp > r): r = temp
    return r
...
def cos_sim(A, B): # (numpy.ndarray, numpy.ndarray) 유사도 출력함수
    ...
    '''
    A_rsp = np.reshape(A[:, :min_a2_b2], (A.shape[0], min_a2_b2))
    print("MFCC shape : ", A_rsp.shape)
    B_rsp = np.reshape(B[:, :min_a2_b2], (B.shape[0], min_a2_b2))
    print("MFCC shape : ", B_rsp.shape)
    '''
...
os.chdir("/Users/jeon-uhyeon/Desktop/final_project/dataset")
path1 = '가수-150-fill.wav' # 파일선택
path2 = '가수-200-fill.wav' # 비교파일선택
# sample_rate=16000
